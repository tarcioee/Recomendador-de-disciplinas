...
def grade_com_resumo_para_linhas(grade, ementario, disciplinas_feitas):
    ...
    # Listar cada disciplina e turma
    for i, (disciplina, turma) in enumerate(grade, 1):
        professores = ', '.join(turma.professores)
        horarios = ', '.join(turma.horarios)
        linhas.append(f"- {disciplina.nome} ({disciplina.codigo}) com {professores} no horário {horarios}")
    ...
    linhas.append("\n📊 Resumo:")
    linhas.append(f"- Dias da semana com aulas: {len(dias_usados)}")

    if total_blocos > 0:
        linhas.append("- Distribuição por turno:")
        for turno in ["matutino", "vespertino", "noturno"]:
            if turnos[turno] > 0:
                perc = (turnos[turno] / total_blocos) * 100
                linhas.append(f"  • {turno.capitalize()}: {perc:.1f}%")
    else:
        linhas.append("- Nenhum turno identificado nos horários.")
    ...
    linhas.append(f"- Carga horária semanal (presença no campus se nao voltar pra casa no meio): {total_horas_semana:.2f} horas")

    contagem_semestre = defaultdict(int)
    for d, _ in grade:
        if d.semestre is not None:
            contagem_semestre[d.semestre] += 1
    linhas.append("- Disciplinas por semestre:")
    for s in sorted(contagem_semestre):
        linhas.append(f"  • Semestre {s}: {contagem_semestre[s]} disciplina(s)")
    ...
    if impacto:
        linhas.append("- Disciplinas com impacto futuro (pré-requisito de obrigatórias ainda não feitas):")
        for cod in impacto:
            ...
            linhas.append(f"  • {nome} ({cod}) – pré de {count} obrigatória(s)")
    else:
        linhas.append("- Nenhuma disciplina da grade é pré-requisito de obrigatórias futuras.")
